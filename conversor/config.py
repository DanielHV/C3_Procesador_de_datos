"""Carga y selección de configuración para el conversor.

Contiene helpers compatibles con los usados por el `agrupador` para permitir
reutilizar selectores (list, regex, meta_filter) sobre el metadata CSV.
"""
...
def _as_number_if_possible(val: Any):
    """Intenta convertir `val` a número (float) y devuelve el valor convertido.

    Si `val` ya es int/float lo devuelve tal cual. Si la conversión falla, devuelve
...
def select_columns_from_metadata(meta_df: pd.DataFrame, selector: Dict[str, Any], id_col: str, alias_col: str) -> List[str]:
    ...
    Soporta los mismos selectores que `agrupador.config.select_columns_from_metadata`:
      - type: list -> values
      - type: regex -> pattern
      - type: meta_filter -> column/op/value
    """
